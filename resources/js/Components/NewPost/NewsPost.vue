<template>
    <div  v-if="visible" class="w-screen h-screen fixed top-0 left-0  z-10 bg-green-500 bg-opacity-40">
        <Toast></Toast>
        <Dialog v-model:visible="visible" header="New News Post" class="sm:w-1/2 w-screen mx-5">
...
function upload(){
    // form.post('blogUpload')
    actions.value = true
    router.post('newsUpload',form,{
        forceFormData: true,
        onSuccess: (data)=>{
        ...
        onError:()=>{
            actions.value = false
            alert('error in uploading the news post')
            console.log('error')
            visible.value = true
        },
        onProgress:(data)=>{
